class Tweet < ActiveRecord::Base
  ...
  def self.findtweets(cat, filter={},page)
    ...
    tweets = Tweet.joins(:tweet_category).where(:tweet_categories=>{:slug => cat}).order('tweets.id DESC').offset(page.to_i * 9).limit(9)
    ...
    if filter.present?
      ...
      if filter['intent'].present?
        ...
        if intentList.include? 'mentioned'
          tweets = tweets.where("has_user_mention = ?", 1)
        end
